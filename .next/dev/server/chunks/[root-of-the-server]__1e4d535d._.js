...
async function GET(req) {
    ...
    const user = users.find((u)=>u.id === userId && u.role === "charity");
    if (!user) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$FWMS$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
    ...
    const items = __TURBOPACK__imported__module__$5b$project$5d2f$FWMS$2f$app$2f$api$2f$_data$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["pickupRequests"].filter((p)=>p.requesterId === user.id && p.requesterType === "charity").sort((a, b)=>b.requestedAt.localeCompare(a.requestedAt)).map((p)=>{
        ...
        // Parse pickupWindow from preferredTime if status is scheduled
        // Format: "YYYY-MM-DD HH:MM-HH:MM" or similar
        let pickupWindow = undefined;
        if (p.status === "scheduled" && p.preferredTime) {
            // Try to parse preferredTime - it might be in format "YYYY-MM-DD HH:MM" or "YYYY-MM-DD HH:MM-HH:MM"
            const timeMatch = p.preferredTime.match(/^(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})(?:-(\d{2}:\d{2}))?$/);
            if (timeMatch) {
                pickupWindow = {
                    date: timeMatch[1],
                    from: timeMatch[2],
                    to: timeMatch[3] || timeMatch[2]
                };
            } else {
...
async function POST(req) {
    ...
    // Support both new format (charityId, itemId) and legacy format (requesterId, wasteId)
    ...
    // Validate requester exists and is a charity user
    const { users } = await __turbopack_context__.A("[project]/FWMS/app/api/_data.ts [app-route] (ecmascript, async loader)");
    const requester = users.find((u)=>u.id === charityId);
    if (!requester) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$FWMS$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
    ...
    const waste = __TURBOPACK__imported__module__$5b$project$5d2f$FWMS$2f$app$2f$api$2f$_data$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["wasteLogs"].find((w)=>w.id === itemId);
    if (!waste) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$FWMS$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
    ...
    // Validate cafeId if provided (new format)
    if (body.cafeId && waste.cafeId !== body.cafeId) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$FWMS$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: "Cafe ID mismatch"
        }, {
            status: 400
        });
    }
    // Validate waste is available for charity
    if (!(waste.type === "edible" && waste.status === "pending" && waste.assignedTo === "charity")) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$FWMS$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1e4d535d._.js.map